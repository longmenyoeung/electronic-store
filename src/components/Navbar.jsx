...
function Navbar() {
  ...
  const menuItems = [
    ...
    return (
        <div className='container px-2 lg:px-[128px] mx-auto py-2 flex justify-between'>
            {/* logo & menu part*/}
            ...
            <ul className='hidden lg:flex'>
                ...
                menuItems.map((item,index)=>(
                    <li key={index} className='mx-5 text-xl hover:text-orange-700 hover:scale-110 duration-300 ease-in-out'>
        ...
        <div className={` ${menuOpen ? 'opacity-100 translate-y-16 pointer-events-auto' : 'opacity-0 -translate-y-14 pointer-events-none' }
            ...
            <ul>
                {
                menuItems.map((item,index)=>(
                    <li key={index} className='py-2 border-b-1 mx-5 text-xl hover:text-orange-700 hover:scale-110 duration-300 ease-in-out'>
                    <NavLink to={item.to}> {item.name} </NavLink>
                    </li>
                ))
                }
